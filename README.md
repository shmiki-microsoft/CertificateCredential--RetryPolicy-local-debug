...
| `DEBUG_PROXY` の値 | 動作 |
| --- | --- |
| `1` / `true` / `on` / `yes` | 既定の `http://127.0.0.1:3128` 経由 |
| `http://host:port` | 指定した URL のプロキシ経由 |
| 未設定 / `0` / `false` / `off` / `no` | プロキシ無効（直接通信） |

```powershell
# プロキシ経由（既定 127.0.0.1:3128）
$env:DEBUG_PROXY = "1"; python main.py
...
| 構成名 | 対象 | `DEBUG_PROXY` |
| --- | --- | --- |
| `Python: main.py` | main.py | `0`（直接通信） |
| `Python: main.py (proxy 127.0.0.1:3128)` | main.py | `1`（プロキシ経由） |
| `Python: main_simple.py` | main_simple.py | `0` |
| `Python: main_no_retry.py` | main_no_retry.py | `0` |

> `env` 内の `DEBUG_PROXY` の値を変えるだけで、デバッグ時のプロキシ経由を切り替えられます。プロキシ経由で検証する場合は、事前に検証用プロキシ（例: Squid）を `3128` で起動しておいてください。
